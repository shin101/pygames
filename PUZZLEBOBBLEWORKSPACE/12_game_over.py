...
class Bubble(pygame.sprite.Sprite):
    def __init__(self, image, color, position=(0,0), row_idx=-1, col_idx=-1):
        ...
        self.radius = 18 #if this number is big, faster ball moves. smaller, slower it gets
...
def process_collision():
    ...
    if hit_bubble or curr_bubble.rect.top <= wall_height:
        row_idx, col_idx = get_map_index(*curr_bubble.rect.center) # * is called unpacking. unpack tuple since out put is a tuple
...
def draw_bubbles():
    ...
    if curr_fire_count == 2:
        to_x = random.randint(0,2) -1 # -1 ~ 1 사이 값이 나옴
